//ЗВЕРНІТЬ УВАГУ!! Під час тестування функцій у вас
// можуть неправильно відображуватись певні символи.
//Це може залежити, як від вашої ОС так і версії Node, це не є критичним
// тому в тестах ви можете використовувати власні приклади.
...
// Завдання 2: Напишіть функцію, яка генерує рядок з символів
// на основі їх кодів Unicode
...
// Завдання 3: Напишіть функцію, яка генерує рядок з символів на основі
// їх кодів Unicode

function generateStringFromCodePoint(code1, code2, code3, code4) {
  // Використовуємо String.fromCodePoint(), щоб створити рядок
  // з символів code1, code2, code3, code4 на основі їх кодів Unicode
...
// Завдання 4: Напишіть функцію, яка виводить рядок
// з використанням String.raw

function rawString() {
  // Використовуємо String.raw, щоб створити рядок
...
// Завдання 6: Напишіть функцію, яка перевіряє,
// чи входить підрядок в рядок

function checkIfIncludes(mainStr, subStr) {
  // Використовуємо метод includes(), щоб перевірити,
  // чи входить підрядок subStr в рядок mainStr
...
function findIndexOf(mainStr, subStr) {
  // Використовуємо метод indexOf(), щоб знайти індекс
  // підрядка subStr в рядку mainStr
...
// Завдання 8: Напишіть функцію, яка знаходить останній індекс
// підрядка в рядку

function findLastIndexOf(mainStr, subStr) {
  // Використовуємо метод lastIndexOf(), щоб знайти останній
  // індекс підрядка subStr в рядку mainStr
...
// Завдання 9: Напишіть функцію, яка перевіряє,
// чи починається рядок з певного підрядка

function checkIfStartsWith(mainStr, subStr) {
  // Використовуємо метод startsWith(), щоб перевірити,
  // чи починається рядок mainStr з певного підрядка subStr
...
// Завдання 10: Напишіть функцію, яка перевіряє,
// чи закінчується рядок певним підрядком

function checkIfEndsWith(mainStr, subStr) {
  // Використовуємо метод endsWith(), щоб перевірити,
  // чи закінчується рядок mainStr певним підрядком subStr
...
// Завдання 11: Напишіть функцію, яка демонструє різницю між методами
// charCodeAt, codePointAt, charAt, at

function compareCharCodeAtAndCodePointAt(string, index) {
  // Використовуємо метод charAt(), щоб отримати символ з рядка
  // на певній позиції, та запишемо результат в змінну charAtResult
  // Використовуємо метод at(), щоб отримати символ з рядка на певній
  // позиції, та запишемо результат в змінну atResult
  // Використовуємо метод charCodeAt(), щоб отримати код символа з рядка
  // на певній позиції, та запишемо результат в змінну charCodeAtResult
  // Використовуємо метод codePointAt(), щоб отримати код символа з рядка
  // на певній позиції, та запишемо результат в змінну codePointAtResult
  // Повертаємо рядок в форматі  `charCodeAt:${charCodeAtResult},
  // codePointAt:${codePointAtResult}, charAt:${charAtResult}, at: ${atResult}`

  const charAtResult = string.charAt();
  const atResult = string.at();
  const charCodeAtResult = string.charCodeAt();
  const codePointAtResult = string.codePointAt();
...
// Завдання 12: Напишіть функцію, яка буде додавати символи до рядка
// з початку або з кінця, в залежності від значення аргументу "side".
...
function padString(original, length, padWith, side) {
  ...
  // Використовуємо метод padEnd(), щоб доповнити рядок string до певної
  // довжини length, вставляючи символи padWith на в кінці
  // Якщо бік не вказано або вказано неправильно, повертаємо
  // повідомлення "Error: side should be either 'start' or 'end'"
...
// Завдання 13: Напишіть функцію, яка буде видаляти пробіли з рядка
// на початку, в кінці або з обох сторін, в залежності
// від значення аргументу "side".
...
function trimString(original, side) {
  ...
  // повертаємо повідомлення "Error: side should be either 'start',
  // 'end' or 'both'"
...
// Завдання 14: Напишіть функцію, яка буде перетворювати рядок в верхній
// або нижній регістр, в залежності від значення аргументу "caseType".
...
function convertCase(original, caseType) {
  ...
  // Якщо значення аргументу "caseType" не є ні "upper", ні "lower",
  // повертаємо повідомлення "Error: caseType should be
...
function repeatString(string, times) {
  // Використовуємо метод repeat(), щоб повторити рядок
  // певну кількість разів times
...
function sliceString(string, startIndex, endIndex) {
  // Використовуємо метод slice(), щоб видалити частину рядка string
  // між двома індексами startIndex та endIndex
...
// Завдання 17: Напишіть функцію, яка видаляє частину рядка між
// двома індексами використовуючи substring

function substringString(string, startIndex, endIndex) {
  // Використовуємо метод substring(), щоб видалити частину рядка
  // string між двома індексами startIndex та endIndex
